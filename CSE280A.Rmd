...
oncogene_compact = filtered_table %>% dplyr::select(which(!colSums(filtered_table[,2:25]) == 0)) %>% mutate(n_total_oncogene = rowSums(filtered_table[,2:25])) %>% arrange(desc(n_total_oncogene)) %>%  filter(n_total_oncogene >= 4) %>% filter(!hgnc_symbol=="MYC")
